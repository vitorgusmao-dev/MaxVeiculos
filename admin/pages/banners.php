...
if ($action === 'set_capa' && $id && isset($_GET['foto_id'])) {
    ...
    try {
        ...
        // Primeiro, resetar a ordem de todas as fotos do veículo
...
// Processar exclusão de foto individual (opcional)
...
if ($action === 'add' || $action === 'edit') {
    ...
    if ($action === 'edit' && $id) {
        ...
        // Obter fotos do veículo ordenadas por ordem (capa será ordem 0)
        ...
        if (empty($marca) || empty($modelo) || empty($ano) || empty($preco)) {
            ...
            try {
                ...
                if (isset($_FILES['fotos']) && !empty($_FILES['fotos']['name'][0])) {
                    // Obter a maior ordem atual para adicionar novas fotos depois
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <small class="text-muted">Você pode selecionar múltiplas imagens (JPG, PNG, GIF).</small>
        </div>

        <!-- Listagem de fotos com opção de definir capa -->
        <?php if (!empty($fotos)): ?>
            <div class="mb-3">
